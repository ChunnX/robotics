...
class Robot:
    ...
    def update_weight(self):
        ...
        for idx in range(self.NUM_OF_PARTICLES):
            x, y, theta = self.particle_set[idx]
            likelihood = calculate_likelihood(x, y, theta, z)
            self.weight[idx] *= likelihood
    ...
    def get_cum_weight(self):
        ...
        for i in range(self.NUM_OF_PARTICLES):
            cum += self.weight[i]
            self.cum_weight[i] = cum

        return self.cum_weight

    def resampling(self):
        self.weight = self.update_weight()
        self.weight = self.normalise_weight()
        self.cum_weight = self.get_cum_weight()

        particles_copy = copy.deepcopy(self.particle_set)

        # generate random number between 0-1
        for idx in range(self.NUM_OF_PARTICLES):
            random_num = random.random()
            for w_idx in range(len(self.cum_weight)):
                if random_num < self.cum_weight[w_idx]:
                    self.particle_set[idx] = particles_copy[w_idx]
                    break
                else:
